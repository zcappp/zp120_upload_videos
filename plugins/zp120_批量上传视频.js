...
function render(ref) {
    ...
    return <React.Fragment>
        ...
        <div className="zp120B">
            <div>{EL.camera}<label>{props.label || "上传视频"}</label></div>
            <input onChange={e => onChange(ref, e)} type="file" accept="video/*" multiple="multiple"/>
        </div>
        {ref.uploading.map((a, i) => <div className={"zp120B zp120U zp120_" + i} key={i}><div/><video src={a}/></div>)}
        {uploaded.map((a, i) => <div className="zp120B zp120Z" onClick={() => {ref.zoom = a; render()}} key={a + i}>
            <img src={a + "?x-oss-process=video/snapshot,m_fast,t_5000,w_0,ar_auto"}/><i className="zplaybtn"/>
...
function onChange(ref, e) {
    ...
    arr.forEach((file, i) => setTimeout(() => {
        ...
        exc('upload(file, onProgress, onSuccess, onError)', {
            file,
            onProgress: r => {
                $("#" + ref.id + " .zp120_" + ref.uploading.indexOf(x) + " div").innerHTML = r.percent + "%"
            },
            onSuccess: r => {
